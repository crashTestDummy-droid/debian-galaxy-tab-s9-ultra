...
/*
 * Send the exact enrolment frame generated by One UI's MDFPP wrapper for an
 * isolated diagnostic UID and a fixed, non-user password.  This never reads
 * Android credentials.  It exists only to validate the QSEECom-compatible
 * transport and map the response schema before the real Ubuntu integration.
 */
static int gatekeeper_send(struct qcomtee_object *controller,
			   const unsigned char *frame, size_t frame_size,
			   unsigned char *response_out, size_t response_capacity,
			   qcomtee_result_t *invoke_result)
{
	struct qcomtee_param params[10] = { 0 };
	unsigned char request[GATEKEEPER_SHARED_SIZE] = { 0 };
	unsigned char response[GATEKEEPER_SHARED_SIZE] = { 0 };
	unsigned char request_out[GATEKEEPER_SHARED_SIZE] = { 0 };
	uint32_t is_64_bit = 1;
	qcomtee_result_t result = QCOMTEE_ERROR;
	size_t i;

	if (frame_size > sizeof(request) ||
	    response_capacity < GATEKEEPER_SHARED_SIZE)
		return -1;
	memset(response_out, 0, response_capacity);
	...
	params[1].ubuf.size = sizeof(response);
	params[2].attr = QCOMTEE_UBUF_INPUT;
	params[2].ubuf.addr = NULL;
	params[2].ubuf.size = 0;
	params[3].attr = QCOMTEE_UBUF_INPUT;
	params[3].ubuf.addr = &is_64_bit;
	params[3].ubuf.size = sizeof(is_64_bit);
	params[4].attr = QCOMTEE_UBUF_OUTPUT;
	...
	params[5].attr = QCOMTEE_UBUF_OUTPUT;
	params[5].ubuf.addr = response_out;
	params[5].ubuf.size = response_capacity;
	for (i = 6; i < 10; i++) {
	...
	if (qcomtee_object_invoke(controller,
				  QSEECOM_COMPAT_SEND_REQUEST_OP,
				  params, 10, &result)) {
		fprintf(stderr, "FAIL: Gatekeeper transport error\n");
		return -1;
	}
...
static int der_length(const unsigned char *buffer, size_t size,
		      size_t *header_size, size_t *value_size)
{
	size_t bytes;
	size_t value = 0;
	size_t i;
	...
	if (!(buffer[1] & 0x80)) {
		*header_size = 2;
		*value_size = buffer[1];
		return *value_size <= size - 2 ? 0 : -1;
	}
	bytes = buffer[1] & 0x7f;
	if (!bytes || bytes > sizeof(size_t) || bytes > size - 2)
		return -1;
	for (i = 0; i < bytes; i++)
		value = (value << 8) | buffer[2 + i];
	*header_size = 2 + bytes;
	*value_size = value;
	return value <= size - *header_size ? 0 : -1;
}

static int gatekeeper_extract_octet(const unsigned char *frame,
				    ...
				    const unsigned char **octet)
{
	...
	if (frame_capacity < 10)
		return -1;
	memcpy(&declared, frame, sizeof(declared));
	if (declared > frame_capacity - 8 || frame[8] != 0x30 ||
	    der_length(frame + 8, (size_t) declared, &sequence_header,
		       &sequence_size))
		return -1;
	...
	while (offset < 8 + sequence_header + sequence_size) {
		...
		size_t inner_header;
		size_t inner_size;
		const unsigned char *outer = frame + offset;
		size_t remaining = 8 + sequence_header + sequence_size - offset;

		if (der_length(outer, remaining, &outer_header, &outer_size))
			return -1;
		if ((outer[0] & 0xe0) == 0xa0 && outer_size >= 2 &&
		    outer[outer_header] == 0x04 &&
		    !der_length(outer + outer_header, outer_size,
...
static int gatekeeper_dummy_enroll(struct qcomtee_object *controller)
{
	static const unsigned char enroll_frame[] = {
		0x35, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x30, 0x33, 0x02, 0x01, 0x03, 0x02, 0x01, 0x64,
		0x02, 0x02, 0x02, 0x01, 0x02, 0x02, 0x4d, 0x29,
		0xa0, 0x06, 0x02, 0x04, 0x47, 0x54, 0x53, 0x55,
		0xa3, 0x02, 0x04, 0x00,
		0xa4, 0x13, 0x04, 0x11,
		'g', 't', 's', '9', 'u', '-', 'd', 'u', 'm', 'm', 'y', '-',
		'p', 'r', 'o', 'b', 'e',
	...
	static const unsigned char verify_prefix[] = {
		0x77, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x30, 0x75, 0x02, 0x01, 0x03, 0x02, 0x01, 0x64,
		0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x5b, 0x3d,
		0xa0, 0x06, 0x02, 0x04, 0x47, 0x54, 0x53, 0x55,
		0xa1, 0x0a, 0x02, 0x08, 0x11, 0x22, 0x33, 0x44,
		0x55, 0x66, 0x77, 0x88,
		0xa3, 0x13, 0x04, 0x11,
		'g', 't', 's', '9', 'u', '-', 'd', 'u', 'm', 'm', 'y', '-',
		'p', 'r', 'o', 'b', 'e',
	...
	unsigned char response[GATEKEEPER_SHARED_SIZE] = { 0 };
	unsigned char verify_frame[sizeof(verify_prefix) + 58] = { 0 };
	const unsigned char *handle;
	const unsigned char *hat;
	qcomtee_result_t result = QCOMTEE_ERROR;

	if (gatekeeper_send(controller, enroll_frame, sizeof(enroll_frame),
			    response, sizeof(response), &result) ||
	    result != QCOMTEE_OK ||
	    gatekeeper_extract_octet(response, sizeof(response), 58, &handle)) {
		fprintf(stderr, "FAIL: diagnostic Gatekeeper enrol did not return a 58-byte handle (invoke=%u)\n",
			result);
		return -1;
	}
	printf("Gatekeeper dummy enrol: invoke=0, handle-size=58.\n");
	memcpy(verify_frame, verify_prefix, sizeof(verify_prefix));
	memcpy(verify_frame + sizeof(verify_prefix), handle, 58);
	memset(response, 0, sizeof(response));
	if (gatekeeper_send(controller, verify_frame, sizeof(verify_frame),
			    response, sizeof(response), &result) ||
	    result != QCOMTEE_OK ||
	    gatekeeper_extract_octet(response, sizeof(response), 69, &hat)) {
		fprintf(stderr, "FAIL: diagnostic Gatekeeper verify did not return a 69-byte HAT (invoke=%u)\n",
			result);
		memset(verify_frame, 0, sizeof(verify_frame));
		memset(response, 0, sizeof(response));
		return -1;
	}
	(void) hat;
	printf("Gatekeeper dummy verify: invoke=0, HAT-size=69; token bytes withheld.\n");
	memset(verify_frame, 0, sizeof(verify_frame));
	memset(response, 0, sizeof(response));
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	int run_prepare = 0;
	int run_gatekeeper_dummy_enroll = 0;
	int i;
	int exit_code = 1;
	...
	for (i = 4; i < argc; i++) {
		if (!strncmp(argv[i], "--type-check", 12)) {
			if (run_type_check || run_prepare || run_gatekeeper_dummy_enroll ||
			    parse_type_check(argv[i], &name_first, &name_last))
				goto usage;
			run_type_check = 1;
		} else if (!strcmp(argv[i], "--prepare")) {
			if (run_prepare || run_type_check || run_gatekeeper_dummy_enroll)
				goto usage;
			run_prepare = 1;
		} else if (!strcmp(argv[i], "--gatekeeper-dummy-enroll")) {
			if (run_prepare || run_type_check || run_gatekeeper_dummy_enroll)
				goto usage;
			run_gatekeeper_dummy_enroll = 1;
	...
	if (result == QCOMTEE_OK && controller != QCOMTEE_OBJECT_NULL) {
		...
		if (run_prepare && prepare_ta(controller, root))
			exit_code = 1;
		if (run_gatekeeper_dummy_enroll &&
		    gatekeeper_dummy_enroll(controller))
			exit_code = 1;
		goto out;
	}
	...
	if (run_prepare && prepare_ta(controller, root))
		exit_code = 1;
	if (run_gatekeeper_dummy_enroll && gatekeeper_dummy_enroll(controller))
		exit_code = 1;
	goto out;

usage:
	fprintf(stderr,
		"usage: %s SPLIT_DIRECTORY BASENAME LOAD_NAME [--type-check[=FIRST[-LAST]] | --prepare | --gatekeeper-dummy-enroll] [--client-uid=UID | --kernel-client-env]\n",
		argv[0]);
	return 64;

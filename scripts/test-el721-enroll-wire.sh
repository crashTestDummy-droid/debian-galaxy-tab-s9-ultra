#!/bin/bash
# Offline, synthetic EnrollDo decoding tests; never opens the reader.
set -euo pipefail
repo=$(cd "$(dirname "$0")/.." && pwd)
test_dir=$(mktemp -d -t el721-enroll-wire.XXXXXX)
...
${CC:-cc} -std=c11 -Wall -Wextra -Werror ${CFLAGS:-} \
  ...
  "$repo/packaging/libfprint/el721-enroll-wire-test.c" \
